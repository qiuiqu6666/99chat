...
- (instancetype)initWithCameraSensor:(PigeonSensorPosition)sensor
  ...
  // Attach microphone before the session starts running so the first
  // startRecording() does not mutate a live session (preview flash).
  if (captureMode == Video) {
    [self setUpCaptureSessionForAudioError:^(NSError *audioError) {
      NSLog(@"camerawesome: audio setup failed: %@", audioError.localizedDescription);
    }];
  }
