...
//                    .build()
                ...
                if (currentCaptureMode == CaptureModes.PHOTO || currentCaptureMode == CaptureModes.VIDEO) {
...
//                Log.d("CameraX", "Surface request result: ${it.resultCode}")
    ...
    fun setCaptureMode(captureMode: CaptureModes) {
        ...
        if (captureMode == CaptureModes.PREVIEW || captureMode == CaptureModes.ANALYSIS_ONLY) {
            videoCaptures.clear()
    ...
    fun hasPhotoAndVideoUseCases(): Boolean {
        return imageCaptures.isNotEmpty() && videoCaptures.isNotEmpty()
    }
