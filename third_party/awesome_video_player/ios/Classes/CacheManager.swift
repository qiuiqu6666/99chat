...
@objc public class CacheManager: NSObject {
    ...
    @objc public func setup(){
        ...
        let proxy = LocalHLSCachingProxy(
            proxyWebServer: webServer,
...
/// 本地 HLS 反代：关闭进后台自动 stop。
/// GCDWebServer 默认会在主线程（User-interactive）里 dispatch_group_wait
/// 等 Default QoS 的 socket 取消回调，触发优先级反转告警。
private final class LocalHLSCachingProxy: HLSCachingReverseProxyServer {
    private static let originQueryKey = "__hls_origin_url"

    private let localWebServer: GCDWebServer
    private var boundPort: Int?

    init(proxyWebServer: GCDWebServer, urlSession: URLSession, cache: PINCaching) {
        self.localWebServer = proxyWebServer
        super.init(webServer: proxyWebServer, urlSession: urlSession, cache: cache)
    }

    override func start(port: UInt) {
        guard !localWebServer.isRunning else { return }
        do {
            try localWebServer.start(options: [
            ...
            NSLog("HLS cache proxy failed to start: \(error.localizedDescription)")
        }
    }
    ...
    override func reverseProxyURL(from originURL: URL) -> URL? {
        ...
        let originItem = URLQueryItem(
            name: LocalHLSCachingProxy.originQueryKey,
            value: originURL.absoluteString
        )
