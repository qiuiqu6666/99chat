...
- (NSObject<FlutterPlatformView>*)createWithFrame:(CGRect)frame
    ...
    BetterPlayer* player = [_players objectForKey:@(textureId.longLongValue)];
    if (player == nil) {
        return nil;
    }
...
- (void) setRemoteCommandsNotificationActive{
    AWVPSetAudioSessionActiveAsync(true, 0);
    [[UIApplication sharedApplication] beginReceivingRemoteControlEvents];
}

- (void) setRemoteCommandsNotificationNotActive{
    if ([_players count] == 0) {
        AWVPSetAudioSessionActiveAsync(false, 0);
    }
...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    if ([@"init" isEqualToString:call.method]) {
        ...
        if ([@"setDataSource" isEqualToString:call.method]) {
            ...
            if ([_players count] == 0) {
                AWVPSetAudioSessionActiveAsync(false, AVAudioSessionSetActiveOptionNotifyOthersOnDeactivation);
            }
            result(nil);
