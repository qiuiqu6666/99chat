...
- (nonnull UIView *)view {
    if (_playerView == nil) {
        ...
        _playerView.opaque = YES;
        _playerView.backgroundColor = UIColor.blackColor;
        _playerView.clipsToBounds = YES;
        _playerView.player = _player;
        _playerView.playerLayer.videoGravity = AVLayerVideoGravityResizeAspect;
        _playerView.playerLayer.backgroundColor = UIColor.blackColor.CGColor;
        _playerView.playerLayer.opaque = YES;
    }
    return _playerView;
}
...
- (AVMutableVideoComposition*)getVideoCompositionWithTransform:(CGAffineTransform)transform
    ...
    if (width < 2 || height < 2 || !isfinite(width) || !isfinite(height)) {
        return nil;
    }
...
- (void)setDataSourcePlayerItem:(AVPlayerItem*)item withKey:(NSString*)key{
    ...
    void (^assetCompletionHandler)(void) = ^{
        ...
        NSURL *url = [asset isKindOfClass:[AVURLAsset class]]
            ? [(AVURLAsset *)asset URL]
            : nil;
        // HTTP(S)/HLS 不能套 videoComposition，否则 Fig -12710：画面灰、声音还在。
        ...
        void (^trackCompletionHandler)(void) = ^{
            ...
            if ([videoTrack statusOfValueForKey:@"preferredTransform" error:nil] != AVKeyValueStatusLoaded ||
                [videoTrack statusOfValueForKey:@"naturalSize" error:nil] != AVKeyValueStatusLoaded ||
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if (item != self->_player.currentItem) {
                    return;
                }
                item.videoComposition = videoComposition;
            });
        };
        [videoTrack loadValuesAsynchronouslyForKeys:@[
            @"preferredTransform",
            @"naturalSize",
            @"nominalFrameRate"
...
- (void)onReadyToPlay {
    if (_eventSink && !_isInitialized && _key) {
        ...
        if (assetTrack != nil &&
            [assetTrack statusOfValueForKey:@"naturalSize" error:nil] == AVKeyValueStatusLoaded) {
            CGSize naturalSize = assetTrack.naturalSize;
            CGAffineTransform prefTrans = assetTrack.preferredTransform;
...
- (void)setupPipController {
    if (@available(iOS 9.0, *)) {
        AWVPSetAudioSessionActiveAsync(YES, 0);
...
- (void)setMixWithOthers:(bool)mixWithOthers {
  if (mixWithOthers) {
    AWVPSetAudioSessionCategoryAsync(AVAudioSessionCategoryPlayback, AVAudioSessionCategoryOptionMixWithOthers);
  } else {
    AWVPSetAudioSessionCategoryAsync(AVAudioSessionCategoryPlayback, 0);
  }
}
