...
class Permissions {
  ...
  /// 非 null 时优先由宿主处理（返回 null 则走 UIKit 默认逻辑）。
  static Future<bool?> Function(BuildContext context, int value)?
      onCheckPermission;
  ...
  static Future<bool> checkPermission(
    ...
    final override = onCheckPermission;
    if (override != null) {
      final custom = await override(context, value);
      if (custom != null) {
        return custom;
      }
    }
    ...
    if (shouldRequestPermission != null && shouldRequestPermission) {
      final result = await Permission.byValue(value).request();
      _entry?.remove();
      _entry = null;
