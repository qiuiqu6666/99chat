...
class _TIMUIKitGroupProfileState extends TIMUIKitState<TIMUIKitGroupProfile> {
  ...
  @override
  void dispose() {
    // Provider.value exposes this State-owned model without owning its lifetime.
    model.dispose();
    super.dispose();
  }
    ...
    return MultiProvider(
        ...
        builder: (context, w) {
          ...
          if (model.backendSelfRole == null &&
            ...
            return Center(
              child: TextButton(
                onPressed: () => model.loadGroupInfo(widget.groupID),
                child: Text(TIM_t('群资料加载失败，点击重试')),
              ),
            );
          }
          ...
          if (needUpdate != null && needUpdate.groupID == widget.groupID) {
            ...
            // Own role is refreshed with detail below. Management pagination
            // belongs to member and management pages, never profile events.
