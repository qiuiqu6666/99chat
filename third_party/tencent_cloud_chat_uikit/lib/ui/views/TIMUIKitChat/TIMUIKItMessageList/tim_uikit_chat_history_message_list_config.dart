...
class TIMUIKitHistoryMessageListConfig {
  ...
  /// Dragging/ballistic scroll cache extent. Keep smaller than [cacheExtent]
  /// so complex chat rows do less speculative build work while the user is
  /// actively scrolling.
  double? scrollingCacheExtent;
  ...
  final bool Function(V2TimMessage message)? isHeavyCustomMessage;

  /// 列表项是否跳过 RepaintBoundary（钱包卡片等自定义重绘边界易出现叠影）。
  final bool Function(V2TimMessage message)? skipRepaintBoundaryForMessage;

  /// 列表 builder 是否为子项自动包 RepaintBoundary（默认 true）。
  final bool? addRepaintBoundaries;

  /// 列表项是否跳过入场 Opacity 动画（与 [skipRepaintBoundaryForMessage] 常配对使用）。
  ...
  TIMUIKitHistoryMessageListConfig(
      ...
      this.shrinkWrap,
      this.clipBehavior,
      this.isHeavyCustomMessage,
      this.skipRepaintBoundaryForMessage,
