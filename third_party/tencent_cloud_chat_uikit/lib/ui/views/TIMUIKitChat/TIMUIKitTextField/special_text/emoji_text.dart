...
class EmojiText extends SpecialText {
  ...
  InlineSpan finishText() {
    ...
    if (emojiUtil.emojiMap.containsKey(key)) {
      return imageSpanForKey(
        key: key,
        assetPath: emojiUtil.emojiMap[key]!,
        textStyle: textStyle,
        start: start!,
        useStickerPackage: (isUseQQPackage == true &&
                (emojiUtil.emojiKeyCategoryMap["4349"]?.contains(key) ??
                    false)) ||
            (isUseTencentCloudChatPackage == true &&
                (emojiUtil.emojiKeyCategoryMap["tcc1"]?.contains(key) ??
                    false)),
      );
    }
  ...
  static InlineSpan? spanForBracketToken({
    ...
    final emojiUtil = EmojiUtil(
      isUseQQPackage: isUseQQPackage,
      isUseTencentCloudChatPackage: isUseTencentCloudChatPackage,
      isUseTencentCloudChatPackageOldKeys: isUseTencentCloudChatPackageOldKeys,
      customEmojiStickerList: customEmojiStickerList,
    );
    final assetPath = emojiUtil.emojiMap[token];
    if (assetPath == null) {
      return null;
    }
    final useStickerPackage = (isUseQQPackage == true &&
            (emojiUtil.emojiKeyCategoryMap["4349"]?.contains(token) ??
                false)) ||
        (isUseTencentCloudChatPackage == true &&
            (emojiUtil.emojiKeyCategoryMap["tcc1"]?.contains(token) ?? false));
    return imageSpanForKey(
      key: token,
      assetPath: assetPath,
      textStyle: textStyle,
      start: start,
      useStickerPackage: useStickerPackage,
    );
  }

  static ImageSpan imageSpanForKey({
    required String key,
    required String assetPath,
    TextStyle? textStyle,
    required int start,
    required bool useStickerPackage,
  }) {
    double size = 16;
    ...
    return ImageSpan(
      AssetImage(
        assetPath,
        package: useStickerPackage ? 'tim_ui_kit_sticker_plugin' : null,
      ),
      actualText: key,
      imageWidth: size,
      imageHeight: size,
