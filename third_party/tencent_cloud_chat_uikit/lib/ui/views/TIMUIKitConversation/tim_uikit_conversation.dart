...
class TIMUIKitConversation extends StatefulWidget {
  ...
  /// String abstract for last message (e.g. CUSTOM previews). Draft priority
  /// stays in [TIMUIKitLastMsg]; prefer this over [lastMessageBuilder] for
  /// conversation list CUSTOM summaries.
  ...
  /// 归档范围；与列表单聊/群聊一致时，仅过滤对应范围的已归档会话。
  final ConversationArchiveScope? archiveScope;

  /// 下拉刷新会话后额外回调（如刷新电脑端登录横幅）。
...
class ConversationItemSlidePanel extends TIMUIKitStatelessWidget {
  ...
  /// Padding of the slidable button content.
...
class _TIMUIKitConversationState extends TIMUIKitState<TIMUIKitConversation> {
  ...
  ConversationArchiveScope? get _activeArchiveScope => widget.archiveScope;

  Set<String> _readArchivedIDsForActiveScope() {
    if (_activeArchiveScope == null) {
      return archivedConversationIDsNotifier.value;
    }
    return archivedConversationIDsNotifierFor(_activeArchiveScope!).value;
  }

  void _handleArchivedConversationIDsChanged() {
    if (!mounted) {
      return;
    }
    setState(() {
      _archivedConversationIDs = _readArchivedIDsForActiveScope();
    });
  }
  ...
  void initState() {
    ...
    archivedConversationC2cIDsNotifier.addListener(
      _handleArchivedConversationIDsChanged,
    );
    archivedConversationGroupIDsNotifier.addListener(
      _handleArchivedConversationIDsChanged,
    );
    ensureArchivedConversationIDsLoaded().then((_) {
      if (mounted) {
        _handleArchivedConversationIDsChanged();
      }
    });
  ...
  Future<void> _archiveConversation(V2TimConversation conversation) async {
    await addArchivedConversation(conversation);
    if (mounted) {
      setState(() {
        _archivedConversationIDs = _readArchivedIDsForActiveScope();
        if (model.selectedConversation?.conversationID ==
  ...
  Future<void> _showMoreActionsSheet(V2TimConversation conversation) async {
        ...
        return CupertinoActionSheet(
          actions: [
            ...
            CupertinoActionSheetAction(
              ...
              child: Text(
                conversation.isPinned! ? TIM_t("取消置顶") : TIM_t("置顶"),
              ),
            ),
            CupertinoActionSheetAction(
              onPressed: () {
  ...
  List<V2TimConversation?> getFilteredConversation() {
    ...
    filteredConversationList = filteredConversationList
        .where((element) =>
            element?.conversationID != null &&
            !_archivedConversationIDs.contains(element!.conversationID))
        .toList();
  ...
  List<ConversationItemSlidePanel> _defaultSlideBuilder(
    ...
    final isDisturb = (conversationItem.recvOpt ?? 0) != 0;
    final disturbLabel = isDisturb ? TIM_t("取消免打扰") : TIM_t("免打扰");
    return [
      if (!PlatformUtils().isWeb)
        ConversationItemSlidePanel(
          onPressed: (context) {
            _showMoreActionsSheet(conversationItem);
          },
          flex: 1,
      ...
      ConversationItemSlidePanel(
        onPressed: (context) {
          _toggleConversationDisturb(conversationItem);
        },
        flex: 1,
                        ...
                        return Material(
                          ...
                          child: GestureDetector(
                            ...
                            child: TIMUIKitConversationItem(
                                ...
                                avatarSize: isDesktopScreen ? 40 : 52,
                                isDisturb:
                                    (conversationItem.groupType == "Meeting"
                                        ? false
                                        : (conversationItem.recvOpt ?? 0) != 0),
                          ...
                          defaultWidget: AutoScrollTag(
                            ...
                            child: Slidable(
                                ...
                                endActionPane: ActionPane(
                                    extentRatio:
                                        slideChildren.length > 2 ? 0.5 : 0.36,
          ...
          return TUIKitScreenUtils.getDeviceWidget(
              ...
              defaultWidget: SlidableAutoCloseBehavior(
                child: EasyRefresh(
                  ...
                  onRefresh: () async {
                    model.refresh();
                    final extra = widget.onPullToRefresh;
                    if (extra != null) {
                      await extra();
                    }
                  },
