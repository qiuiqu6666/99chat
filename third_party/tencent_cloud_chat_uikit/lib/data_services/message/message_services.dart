...
/// Structured result for the complete history API.
///
/// The legacy method only returns the payload and therefore turns a native
/// SDK error into `null`.  The IM history coordinator needs the original code
/// and description to distinguish an exhausted page from a retryable failure.
class MessageHistorySdkResult {
  const MessageHistorySdkResult({
    required this.code,
    required this.desc,
    this.data,
  });
  ...
  final int code;
  final String desc;
  final V2TimMessageListResult? data;
...
abstract class MessageService {
  ...
  /// Structured companion API. Existing implementations and test doubles can
  /// keep implementing the legacy method; the default adapter preserves that
  /// compatibility while production implementations expose native status.
  Future<MessageHistorySdkResult> getHistoryMessageListWithStatus({
    ...
    final data = await getHistoryMessageListWithComplete(
      getType: getType,
      userID: userID,
      groupID: groupID,
    ...
    return MessageHistorySdkResult(
      code: data == null ? -1 : 0,
      desc: data == null ? 'history result unavailable' : 'OK',
      data: data,
    );
  }
  ...
  /// Sends an ephemeral C2C typing signal independently of the message Outbox
  /// and serial send queue. Failures are disposable and never become bubbles.
