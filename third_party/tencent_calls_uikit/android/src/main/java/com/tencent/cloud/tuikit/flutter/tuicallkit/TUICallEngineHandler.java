...
public class TUICallEngineHandler {
    ...
    private volatile String activeIncomingCallId = "";
    private volatile String activeIncomingCallerId = "";

    private void clearIncomingCallNotification(String callId) {
        try {
            IncomingNotificationView.getInstance(mApplicationContext).cancelNotification(callId);
            if (callId != null && callId.equals(activeIncomingCallId)) {
                activeIncomingCallId = "";
                activeIncomingCallerId = "";
            }
        } catch (Exception e) {
            Logger.warning(TUICallKitPlugin.TAG, "clearIncomingCallNotification error:" + e);
        }
    }
    ...
    private TUICallObserver mTUICallObserver = new TUICallObserver() {
        ...
        public void onCallReceived(String callId, String callerId, List<String> calleeIdList, TUICallDefine.MediaType mediaType, TUICallDefine.CallObserverExtraInfo info) {
            activeIncomingCallId = callId == null ? "" : callId;
            activeIncomingCallerId = callerId == null ? "" : callerId;
            IncomingNotificationView.getInstance(mApplicationContext).setActiveCallId(activeIncomingCallId);
        ...
        public void onCallCancelled(String callerId) {
            if (callerId != null && callerId.equals(activeIncomingCallerId)) {
                clearIncomingCallNotification(activeIncomingCallId);
            }
