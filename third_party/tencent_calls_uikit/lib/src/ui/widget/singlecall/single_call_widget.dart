...
import 'package:tencent_calls_uikit/src/ui/widget/common/call_avatar.dart';
...
class _SingleCallWidgetState extends State<SingleCallWidget> {
  ...
  User? _displayPeer() {
    if (CallState.instance.remoteUserList.isNotEmpty) {
      return CallState.instance.remoteUserList[0];
    }
    if (CallState.instance.caller.id.isNotEmpty &&
        CallState.instance.caller.id != CallState.instance.selfUser.id) {
      return CallState.instance.caller;
    }
    if (CallState.instance.calleeList.isNotEmpty) {
      return CallState.instance.calleeList[0];
    }
    return null;
  }
  ...
  _buildBackground() {
    var avatar = '';
    final peer = _displayPeer();
    if (peer != null) {
      avatar = StringStream.makeNull(peer.avatar, Constants.defaultAvatar);
    }
    ...
    return Stack(
      ...
      children: [
        Positioned.fill(
          child: CallAvatarImage(url: avatar, fit: BoxFit.cover),
        ),
        Opacity(
            opacity: 1,
            child: Container(
  ...
  _buildUserInfoWidget() {
    ...
    final peer = _displayPeer();
    if (peer != null) {
      showName = User.getUserDisplayName(peer);
      avatar = StringStream.makeNull(peer.avatar, Constants.defaultAvatar);
    }
    ...
    final userInfoWidget = Positioned(
        ...
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            CallAvatarView(url: avatar, size: 110),
                  ...
                  CallState.instance.selfUser.callStatus == TUICallStatus.accept
                      ? Visibility(
                          ...
                          child: Center(
                            ...
                            child: CallAvatarView(
                              url: isLocalViewBig ? selfAvatar : remoteAvatar,
                              size: 80,
                            ),
                          )),
                        )
                ...
                Visibility(
                  ...
                  child: Center(
                    child: Container(
                      ...
                      child: CallAvatarView(
                        url: isRemoteViewSmall ? remoteAvatar : selfAvatar,
                        size: 80,
                      ),
                    ),
                  ),
