...
class _GroupCallWidgetState extends State<GroupCallWidget> {
  TUIAudioPlaybackDevice _deviceForRoute(VoiceOutputRoute route) {
    return route == VoiceOutputRoute.speaker
        ? TUIAudioPlaybackDevice.speakerphone
        : TUIAudioPlaybackDevice.earpiece;
  }

  VoiceOutputRoute _targetRouteForCurrentDevice() {
    return CallState.instance.audioDevice == TUIAudioPlaybackDevice.earpiece
          ...
          children: [
            Container(
              ...
              child: CallAvatarView(
                url: CallState.instance.caller.avatar,
                size: 120,
              ),
            ),
            Padding(
            ...
            Container(
              ...
              child: Wrap(
                ...
                children: List.generate(CallState.instance.calleeList.length,
                    ((index) {
                  return CallAvatarView(
                    url: CallState.instance.calleeList[index].avatar,
                    size: 30,
                  );
                })),
              ),
  ...
  _handleSwitchAudioDevice() async {
    final targetRoute = _targetRouteForCurrentDevice();
  ...
  _handleHangUp(Function close) async {
    final result = await CallManager.instance.hangup();
  ...
  _handleAccept() async {
    ...
    if (permissionRequestResult == PermissionResult.granted || Platform.isIOS) {
      ...
      if (result.code.isNotEmpty) {
        CallManager.instance.showToast(result.message ?? result.code);
      }
    } else {
