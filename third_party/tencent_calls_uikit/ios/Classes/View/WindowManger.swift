...
class WindowManger: NSObject, FloatWindowViewDelegate, IncomingBannerViewDelegate {
    ...
    func isCallFloatWindowVisible() -> Bool {
        guard let root = floatWindow.rootViewController else {
            return false
        }
        return root is FloatWindowViewController && !floatWindow.isHidden
    }

    func activeFloatRemoteVideoView() -> UIView? {
        guard isCallFloatWindowVisible(),
              let controller = floatWindow.rootViewController as? FloatWindowViewController else {
            return nil
        }
        return controller.activeRemoteVideoView()
    }

    func pipHostWindow() -> UIWindow? {
        guard isCallFloatWindowVisible() else {
            return nil
        }
        return floatWindow
    }
    ...
    func closeIncomingBannerView() {
        if let _ = floatWindow.rootViewController as? FloatWindowViewController {
            return
        }
