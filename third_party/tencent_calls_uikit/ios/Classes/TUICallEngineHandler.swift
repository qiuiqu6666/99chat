...
extension TUICallEngineHandler {
    ...
    func startRemoteView(call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        for index in 0..<TUICallState.instance.remoteUserList.value.count {
            if TUICallState.instance.remoteUserList.value[index].id.value == userId {
                TUICallState.instance.remoteUserList.value[index].viewID.value = viewId
                break
            }
        }
        ...
        TUICallEngine.createInstance().startRemoteView(userId: userId, videoView: videoView) { userId in
            CallPipLogger.log("startRemoteView success userId=\(userId) viewId=\(viewId)")
            for index in 0..<TUICallState.instance.remoteUserList.value.count {
                if TUICallState.instance.remoteUserList.value[index].id.value == userId {
                    TUICallState.instance.remoteUserList.value[index].viewID.value = viewId
                    break
            ...
            CallPipLogger.log("startRemoteView error userId=\(userId) code=\(code) message=\(message ?? "")")
        }
        result(NSNumber(value: 0))
    }
