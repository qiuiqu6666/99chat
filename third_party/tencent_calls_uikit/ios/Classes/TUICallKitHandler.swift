...
class TUICallKitHandler {
    ...
    enum Mehtod: String {
        ...
        case closeIncomingBanner
        case enableWakeLock
        case setScreenPowerPolicy
        case loginNativeTUICore
        case logoutNativeTUICore
        case enterMobilePictureInPicture
        case isAndroidPictureInPictureSupported
    }
    ...
    init(registrar: FlutterPluginRegistrar) {
        ...
        channel.setMethodCallHandler({[weak self] call, result in
            ...
            case .closeIncomingBanner:
                closeIncomingBanner(call: call, result: result)
            case .enableWakeLock:
                enableWakeLock(call: call, result: result)
            case .setScreenPowerPolicy:
                setScreenPowerPolicy(call: call, result: result)
            case .loginNativeTUICore:
                loginNativeTUICore(call: call, result: result)
            case .logoutNativeTUICore:
                logoutNativeTUICore(call: call, result: result)
            case .enterMobilePictureInPicture:
                enterMobilePictureInPicture(call: call, result: result)
            case .isAndroidPictureInPictureSupported:
                result(false)
            case .none:
                CallPipLogger.log("unhandled method channel call: \(call.method)")
...
extension TUICallKitHandler {
    ...
    func updateCallStateToNative(call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        if let sceneIndex = MethodUtils.getMethodParams(call: call, key: "scene", resultType: Int.self)  {
            TUICallState.instance.scene.value = UInt(sceneIndex) == 0 || UInt(sceneIndex) == 3  ? .single : .group;
        }
        ...
        CallPipLogger.log(
            "updateCallStateToNative status=\(callStatus.rawValue) mediaType=\(mediaType.rawValue) remoteId=\(remoteId)"
        )
        if callStatus == .accept && mediaType == .video {
            CallPictureInPictureBridge.prepareIfNeeded()
        } else if callStatus == .accept && mediaType == .audio {
            CallPictureInPictureBridge.teardown()
    ...
    private func setUserInfo(dic: Dictionary<String, Any>, isSelfUser: Bool) {
        ...
        if let viewID = dic["viewID"] as? Int {
            if isSelfUser {
                TUICallState.instance.selfUser.value.viewID.value = viewID
            } else {
                if remoteIndex == -1 {
                    remoteUser.viewID.value = viewID
                } else {
                    TUICallState.instance.remoteUserList.value[remoteIndex].viewID.value = viewID
                }
            }
        }
    ...
    func startFloatWindow(call: FlutterMethodCall, result: @escaping FlutterResult) {
        CallPipLogger.log("startFloatWindow begin")
        WindowManger.instance.showFloatWindow()
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.15) {
            CallPipLogger.log(
                "startFloatWindow relocate floatVisible=\(WindowManger.instance.isCallFloatWindowVisible())"
            )
            CallPictureInPictureBridge.relocateForFloatWindowIfNeeded()
            CallPictureInPictureBridge.prepareIfNeeded()
        }
        result(NSNumber(value: 0))
    }
    ...
    func enterMobilePictureInPicture(call: FlutterMethodCall, result: @escaping FlutterResult) {
        let mediaType = TUICallState.instance.mediaType.value
        if mediaType == .audio {
            CallPipLogger.log("enterMobilePictureInPicture skipped: audio uses CallKit Dynamic Island only")
            result(NSNumber(value: 0))
            return
        }
        CallPipLogger.log("enterMobilePictureInPicture from Dart")
    ...
    func closeIncomingBanner(call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    func enableWakeLock(call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        WakeLock.shareInstance().applyPolicy(enable ? .keepAwake : .off)
        result(NSNumber(value: 0))
    }

    func setScreenPowerPolicy(call: FlutterMethodCall, result: @escaping FlutterResult) {
        guard let policyIndex = MethodUtils.getMethodParams(call: call, key: "policy", resultType: Int.self) else {
            FlutterResultUtils.handleMethod(code: .paramNotFound, methodName: "setScreenPowerPolicy", paramKey: "policy", result: result)
            result(NSNumber(value: -1))
            return
        }

        let policy = CallScreenPowerPolicy(rawValue: policyIndex) ?? .off
        WakeLock.shareInstance().applyPolicy(policy)
        result(NSNumber(value: 0))
    }
