...
class EventType {
  ...
  /// TIM Web：长连接断开 / 未就绪（与 SDK_READY 成对）。
  static String SDK_NOT_READY = checkEmptyEnum(
      () => jsToMap(EventEnum.EVENT)['SDK_NOT_READY'], "sdkStateNotReady");

  /// TIM Web：底层网络态 `connected` / `connecting` / `disconnected`。
  static String NET_STATE_CHANGE = checkEmptyEnum(
      () => jsToMap(EventEnum.EVENT)['NET_STATE_CHANGE'], "netStateChange");
