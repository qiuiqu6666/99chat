...
class V2TIMMessage {
  ...
  static String _stripConversationPrefix(dynamic raw) {
  ...
  static String _resolveC2CPeerID(Map<String, dynamic> message) {
    final conversationID = _stripConversationPrefix(message['conversationID']);
    if (conversationID.isNotEmpty) {
      return conversationID;
    }
    final flow = message['flow']?.toString();
    final to = _stripConversationPrefix(message['to']);
    final from = _stripConversationPrefix(
      message['from'] ?? message['fromAccount'] ?? message['messageSender'],
    );
    if (flow == 'out' && to.isNotEmpty) {
      return to;
    }
    if (from.isNotEmpty) {
      return from;
  ...
  static String _resolveGroupID(Map<String, dynamic> message) {
    final to = _stripConversationPrefix(message['to']);
    if (to.isNotEmpty) {
      return to;
    }
    return _stripConversationPrefix(message['conversationID']);
  }

  static Future<Map<String, dynamic>> convertMessageFromWebToDart(jsMessage,
      {isFromDownloadMergerMessage = false}) async {
    final rawMessage = jsMessage is Map ? jsMessage : jsToMap(jsMessage);
    ...
    final formatedMsg = <String, dynamic>{
      ...
      "groupID": message["conversationType"] == "GROUP" ? _resolveGroupID(message) : null,
      "userID": message["conversationType"] == "C2C" ? _resolveC2CPeerID(message) : null,
