...
class GetConversationList {
  ...
  static Map<String, dynamic> formatReturn(List<dynamic> list) {
    return {
      // Web returns the full list in one shot; empty nextSeq stops app-side paging.
      "nextSeq": "",
      "isFinished": true,
  ...
  static formateConversationList(List conversationListJs) async {
    ...
    for (var i = 0; i < formatedConversationList.length; i++) {
      try {
        final item = await formateConversationListItem(
            jsToMap(formatedConversationList[i]));
        conversationList.add(item);
      } catch (e, st) {
        // Skip a single bad conversation instead of failing the whole list.
        // ignore: avoid_print
        print('GetConversationList.formateConversationListItem failed: $e\n$st');
      }
    }
  ...
  static formateConversationListItem(Map<String, dynamic> itemJS) async {
    Map<String, dynamic> conversationListItem = Map<String, dynamic>.from(itemJS);
    final originalConversationID =
        (itemJS['conversationID'] ?? '').toString();
    ...
    conversationListItem["recvOpt"] =
        GroupRecvMsgOpt.convertMsgRecvOpt(
            (itemJS["messageRemindType"] ?? "").toString());
    ...
    if (itemJS['lastMessage'] != null) {
      conversationListItem["lastMessage"] = await formateLasteMessage(
          ...
          groupID: originalConversationID);
    }
    ...
    conversationListItem["type"] =
        ConversationTypeWeb.convertConverstationtType(
            (itemJS["type"] ?? "").toString());
  ...
  static formateLasteMessage(Map<String, dynamic> message,
    ...
    if (elementType == MessageElemType.V2TIM_ELEM_TYPE_GROUP_TIPS) {
      try {
        ...
        formatedMsg['elemType'] = MessageElemType.V2TIM_ELEM_TYPE_NONE;
      }
    }

    // 图片消息
    if (elementType == MessageElemType.V2TIM_ELEM_TYPE_IMAGE) {
      final rawImageInfo = messagePayload['imageInfoArray'];
      final imageInfoArray = rawImageInfo is List
          ? rawImageInfo
          : (rawImageInfo == null
              ? const <dynamic>[]
              : List<dynamic>.from(rawImageInfo as Iterable));
  ...
  static String getShowName(Map<String, dynamic> itemJS) {
    if (itemJS['remark'] != null && itemJS['remark'] != "") {
      return itemJS['remark'].toString();
    } else if (itemJS['groupProfile'] != null) {
      final groupProfile = itemJS['groupProfile'] is Map
          ? Map<String, dynamic>.from(itemJS['groupProfile'] as Map)
          : jsToMap(itemJS['groupProfile']);
      return (groupProfile['name'] ?? groupProfile['groupID'] ?? '').toString();
    } else {
      final userProfile = itemJS["userProfile"] == null
          ? <String, dynamic>{}
          : (itemJS["userProfile"] is Map
              ? Map<String, dynamic>.from(itemJS["userProfile"] as Map)
              : jsToMap(itemJS["userProfile"]));
      final nick = userProfile['nick'];
      final hasNickName = nick != null && nick.toString().isNotEmpty;
      return hasNickName
          ? nick.toString()
