...
class V2TIMGroupManager {
  /// Live TIM JS instance as [dynamic] (avoid false-null DDC casts).
  dynamic get timeweb => V2TIMManagerWeb.timWeb;
  ...
  Future<V2TimCallback> setGroupInfo({
    ...
    try {
      final updateGroupParams = <String, dynamic>{
        "groupID": info.groupID,
      };
      final groupName = info.groupName?.trim();
      if (groupName != null && groupName.isNotEmpty) {
        updateGroupParams["name"] = groupName;
      }
      if (info.faceUrl != null) {
        updateGroupParams["avatar"] = info.faceUrl;
      }
      if (info.introduction != null) {
        updateGroupParams["introduction"] = info.introduction;
      }
      if (info.notification != null) {
        updateGroupParams["notification"] = info.notification;
      }
      if (info.groupAddOpt != null) {
        updateGroupParams["joinOption"] =
            GroupAddOptWeb.convertGroupAddOptToWeb(info.groupAddOpt);
      }
      final customInfo = info.customInfo;
      if (customInfo != null && customInfo.isNotEmpty) {
        updateGroupParams["groupCustomField"] =
            V2TimGroupCreate.convertGroupCustomInfoFromDartToWeb(customInfo);
      }
      if (updateGroupParams.length <= 1) {
