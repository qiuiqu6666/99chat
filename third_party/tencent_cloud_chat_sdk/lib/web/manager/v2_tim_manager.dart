...
import 'v2_tim_conversation_manager.dart';
...
class V2TIMManager {
  ...
  static bool _webSdkEventListenersBound = false;
  ...
  void _bindWebSdkEventListeners(V2TimSDKListener? listener) {
    if (V2TIMManagerWeb.timWeb == null || listener == null) {
      return;
    }
    V2TIMManagerWeb.timWeb!.on(EventType.SDK_READY, allowInterop((res) {
      // Conversation / message listeners may have been registered before TIM was
      // ready, or may have gone stale after reconnect / hot reload.
      // Message listeners are rebound via app onConnectSuccess →
      // TimWebScriptLoader.rebindRealtimeListeners (avoids circular imports).
    ...
    // 断线/重连：原先只绑了 SDK_READY，WebSocket 抖动时 Flutter 层感知不到，
    // 导致漏推消息且不会触发历史补拉。
    V2TIMManagerWeb.timWeb!.on(EventType.SDK_NOT_READY, allowInterop((res) {
      listener.onConnecting();
    }));

    V2TIMManagerWeb.timWeb!.on(EventType.NET_STATE_CHANGE, allowInterop((res) {
      try {
        final mapped = jsToMap(res);
        final dynamic rawState = mapped['state'] ??
            (mapped['data'] is Map ? (mapped['data'] as Map)['state'] : null);
        final state = '$rawState'.trim().toLowerCase();
        if (state == 'disconnected' || state == 'connecting') {
          // 短暂断线用 connecting，避免 onConnectFailed 把已登录态打成 failed。
          listener.onConnecting();
          return;
        }
        if (state == 'connected') {
    ...
    V2TIMManagerWeb.timWeb!.on(EventType.PROFILE_UPDATED, _profileUpdateHandler);
    _webSdkEventListenersBound = true;
  }

  /// App 层（如 reassemble）可主动触发 Web 会话监听自愈。
  /// 消息监听请同步调用 [V2TIMMessageManager.rebindJsMessageListeners]。
  static void rebindWebRealtimeListeners({bool force = true}) {
    V2TIMConversationManager.ensureJsListenerBound(force: force);
  }
  ...
  V2TimValueCallback<bool> initSDK(
    ...
    final timParams = TimParams()..SDKAppID = sdkAppID;
    V2TIMManagerWeb.initWebTim(timParams);
    if (!_webSdkEventListenersBound) {
      _bindWebSdkEventListeners(listener);
    }
  ...
  Future<dynamic> unInitSDK() async {
    if (V2TIMManagerWeb.timWeb != null) {
      try {
        ...
        loginUserID = "";
        V2TIMManagerWeb.resetWebTimState();
        _webSdkEventListenersBound = false;
        V2TIMConversationManager.resetJsListenerBound();
        if (!completer.isCompleted) {
