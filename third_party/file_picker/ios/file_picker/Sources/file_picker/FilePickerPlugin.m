...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    if([call.method isEqualToString:@"clear"]) {
        _result([NSNumber numberWithBool: [FLTFilePickerUtils clearTemporaryFiles]]);
        _result = nil;
        return;
    }
    ...
    if([call.method isEqualToString:@"any"] || [call.method containsString:@"custom"]) {
        self.allowedExtensions = [FLTFilePickerUtils resolveType:call.method withAllowedExtensions: [arguments valueForKey:@"allowedExtensions"]];
...
#ifdef PICKER_MEDIA
        [self resolvePickMedia:[FLTFilePickerUtils resolveMediaType:call.method] withMultiPick:isMultiplePick withCompressionAllowed:self.allowCompression];
#else
        _result([FlutterError errorWithCode:@"Unsupported picker type"
...
- (void) handleResult:(id) files {
    _result([FLTFilePickerUtils resolveFileInfo: [files isKindOfClass: [NSArray class]] ? files : @[files] withData:self.loadDataToMemory]);
    _result = nil;
}
...
- (void)mediaPicker: (MPMediaPickerController *)mediaPicker didPickMediaItems:(MPMediaItemCollection *)mediaItemCollection
{
    ...
    for(MPMediaItemCollection * item in [mediaItemCollection items]) {
        NSURL * cachedAsset = [FLTFilePickerUtils exportMusicAsset: [item valueForKey:MPMediaItemPropertyAssetURL] withName: [item valueForKey:MPMediaItemPropertyTitle]];
