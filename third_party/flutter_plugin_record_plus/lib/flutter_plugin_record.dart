...
class FlutterPluginRecord {
  ...
  static Future<dynamic> _handler(MethodCall methodCall) async {
    ...
    final recordPlugin = alis[id];
    if (recordPlugin == null) {
      return null;
    }
    final sessionId = arguments['sessionId']?.toString();
      ...
      case "onStop":
        if ("success" == methodCall.arguments["result"]) {
          RecordResponse res = new RecordResponse(
            ...
            audioTimeLength: double.tryParse(
              methodCall.arguments["audioTimeLength"]?.toString() ?? '',
            ),
            msg: "onStop",
            key: methodCall.arguments["key"]?.toString() ?? "",
            sessionId: sessionId,
  ...
  Future start({String? sessionId}) async {
  ...
  Future startByWavPath(String wavPath, {String? sessionId}) async {
  ...
  Future stop({String? sessionId}) async {
