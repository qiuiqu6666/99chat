...
void main() {
  ...
  Future<void> receive(WidgetTester tester, int from, int count) async {
    ...
    while (!done && elapsed.elapsed < const Duration(seconds: 30)) {
      await frame(tester);
    }
  ...
  // Real SQLite admissions and the production capsule; no injected counters.
  Future<T> complete<T>(WidgetTester tester, Future<T> operation) async {
    var done = false;
    Object? error;
    T? result;
    final pending = operation.then<void>((value) {
      result = value;
      done = true;
    ...
    expect(done, isTrue, reason: 'SQLite operation must complete');
    await pending;
    expect(error, isNull);
    return result as T;
  }

  Future<HistoryWindowDeferredState> sqlState(WidgetTester tester) => complete(
      tester, store.deferredState(global.historyWindowScopeFor(getConv())!));

  Future<void> beginVisit(WidgetTester tester) =>
      complete(tester, global.beginHistoryUnreadVisit(getConv()));

  Future<void> assertArrivalReminder(WidgetTester tester) async {
    final sql = await sqlState(tester);
    expect(sql.receivedCount, 1,
        reason: 'real durable producer persisted arrival');
    expect(global.receivedNewMessageCountFor(getConv()), 1,
        reason:
            'legacy/scalar count confirms arrival in this initialized visit');
    expect(global.deferredIncomingBufferedCount(getConv()), 1);
    expect(global.remainingLiveIncomingCountFor(getConv()), 1,
        reason:
            'production capsule now uses the identity ledger, which must agree');
    expect(find.text('showUnread:1').hitTestable(), findsOneWidget);
  }

  uiTest('durable newer-gap arrival enters capsule identity ledger',
      (tester) async {
    await mount(tester);
  ...
  uiTest('background arrival while reading enters capsule identity ledger',
      (tester) async {
    await mount(tester);
    await beginVisit(tester);
    await away(tester);
  ...
  uiTest('foreground in-memory arrival updates capsule ledger', (tester) async {
    await mount(tester);
    await beginVisit(tester);
    await away(tester);
    await receive(tester, 101, 1);
  ...
  uiTest(
      'mixed paths and replay count each identity and receive generation once',
      (tester) async {
    await mount(tester);
    await beginVisit(tester);
    await away(tester);
    await receive(tester, 101, 1);
    expect(global.liveReceiveGenerationFor(getConv()), 1);
    global.markMemoryWindowMissingNewer(getConv());
    await receive(tester, 101, 2);
    await receive(tester, 102, 1);
    expect((await sqlState(tester)).receivedCount, 2);
    expect(global.remainingLiveIncomingIdsFor(getConv()),
        {'${getConv()}-101', '${getConv()}-102'});
    expect(global.liveReceiveGenerationFor(getConv()), 2,
        reason: 'moving an existing identity to SQL is not another arrival');
  ...
  uiTest('visible SQL ACK retires the same identity from the capsule ledger',
    ...
    expect(global.remainingLiveIncomingCountFor(getConv()), 2,
        reason: 'stale viewport proof must not consume a reminder');
    expect(
        await complete(
            tester,
            global.acknowledgeVisibleHistoryMessages(getConv(), [row(101)],
  ...
  uiTest(
      'new visit baselines old live IDs without acknowledging their SQL rows',
      (tester) async {
    await mount(tester);
    await beginVisit(tester);
    await awayInHistoryGap(tester);
    ...
    expect((await sqlState(tester)).receivedCount, 2);
    await receive(tester, 101, 3);
    expect(global.remainingLiveIncomingIdsFor(getConv()), {'${getConv()}-103'});
    expect(global.liveReceiveGenerationFor(getConv()), 3);
    expect((await sqlState(tester)).receivedCount, 3);
  });

  uiTest('admission completing after a new visit belongs only to its baseline',
      (tester) async {
    await mount(tester);
    await beginVisit(tester);
    await awayInHistoryGap(tester);
    ...
    final admission = global.applyAppRealtimeMessage(row(101),
        ingressEventID: '${getConv()}-late-101', ingressSequence: 101);
    try {
      for (var attempt = 0;
          attempt < 1500 && !appended.isCompleted;
          attempt++) {
        await frame(tester);
      }
      expect(appended.isCompleted, isTrue);
      final nextVisit = global.beginHistoryUnreadVisit(getConv());
      release.complete();
      await complete(tester, Future.wait([admission, nextVisit]));
    } finally {
      if (!release.isCompleted) release.complete();
    }
    expect(global.remainingLiveIncomingCountFor(getConv()), 0);
    expect(global.liveReceiveGenerationFor(getConv()), 0);
    expect((await sqlState(tester)).receivedCount, 1);
    final watermark = await complete(
        tester, global.beginHistoryWindowReturnToLatest(getConv()));
    await complete(tester,
        global.acknowledgeHistoryWindowReturnToLatest(getConv(), watermark));
    expect((await sqlState(tester)).receivedCount, 0);
    await complete(
        tester,
        global.applyAppRealtimeMessage(row(101),
            ingressEventID: '${getConv()}-replayed-101',
            ingressSequence: 1001));
    expect(global.remainingLiveIncomingCountFor(getConv()), 0,
        reason:
            'ACK may remove SQL identities but cannot turn a baseline replay live');
    expect(global.liveReceiveGenerationFor(getConv()), 0);
    await receive(tester, 102, 1);
    expect(global.remainingLiveIncomingIdsFor(getConv()), {'${getConv()}-102'});
    expect(global.liveReceiveGenerationFor(getConv()), 1);
  ...
  uiTest('accepted deletion retires a durable reminder identity',
      (tester) async {
    await mount(tester);
    await beginVisit(tester);
    await awayInHistoryGap(tester);
    await receive(tester, 101, 2);
    global.retireDeletedLocalIncoming(getConv(), ['${getConv()}-101']);
    expect(global.remainingLiveIncomingIdsFor(getConv()), {'${getConv()}-102'});
    await receive(tester, 101, 1);
    expect(global.remainingLiveIncomingCountFor(getConv()), 1);
    expect(global.liveReceiveGenerationFor(getConv()), 2);
  ...
  uiTest(
      'return to latest consumes identities and duplicate replay stays cleared',
      (tester) async {
    await mount(tester);
    await beginVisit(tester);
    await awayInHistoryGap(tester);
    ...
    expect(global.remainingLiveIncomingCountFor(getConv()), 0,
        reason:
            'ordinary replay after durable ACK must retain seen identities');
    expect(global.liveReceiveGenerationFor(getConv()), 2);
  });
}
