// Patched copy of media_kit_video 2.0.1 windows/media_kit_video_plugin.cc.
// The upstream plugin keeps a process-global WndProc singleton. A second
// Flutter engine (desktop_multi_window child) overwrites it and routes the
// main window's messages through the child's original proc, which kills the
// process ("Lost connection to device").
//
// This copy dispatches by HWND so each engine keeps its own WndProc chain.
// Keep in sync with media_kit_video besides the HWND map.
...
std::mutex g_plugin_by_hwnd_mutex;
std::unordered_map<HWND, MediaKitVideoPlugin*> g_plugin_by_hwnd;

MediaKitVideoPlugin* PluginForWindow(HWND hwnd) {
  std::lock_guard<std::mutex> lock(g_plugin_by_hwnd_mutex);
  auto it = g_plugin_by_hwnd.find(hwnd);
  if (it == g_plugin_by_hwnd.end()) {
    return nullptr;
  }
  return it->second;
}
...
MediaKitVideoPlugin::MediaKitVideoPlugin(
  ...
  {
    std::lock_guard<std::mutex> lock(g_plugin_by_hwnd_mutex);
    g_plugin_by_hwnd[flutter_window_] = this;
  }
...
MediaKitVideoPlugin::~MediaKitVideoPlugin() {
  if (flutter_window_) {
    std::lock_guard<std::mutex> lock(g_plugin_by_hwnd_mutex);
    auto it = g_plugin_by_hwnd.find(flutter_window_);
    if (it != g_plugin_by_hwnd.end() && it->second == this) {
      g_plugin_by_hwnd.erase(it);
    }
  }
...
LRESULT CALLBACK MediaKitVideoPlugin::WindowProcDelegate(HWND hwnd,
                                                         ...
                                                         WPARAM wParam,
                                                         LPARAM lParam) {
  MediaKitVideoPlugin* plugin = PluginForWindow(hwnd);
  if (message == kMainThreadTaskMessage && plugin) {
    plugin->ProcessMainThreadTasks();
    return 0;
  }

  if (plugin && plugin->original_window_proc_) {
    return ::CallWindowProc(plugin->original_window_proc_, hwnd, message,
                            wParam, lParam);
  }
