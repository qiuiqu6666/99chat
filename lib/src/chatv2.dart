...
class _ChatV2State extends State<ChatV2> {
  ...
  Future<bool> loadHistoryMessageList(
    ...
    if (_haveMoreData) {
      final conversationID = _getConvID() ?? '';
      await ConversationHistorySyncCoordinator.instance
          .runUserOlderPagination(
        conversationID: conversationID,
        task: () async {
          _haveMoreData = await _controller.loadHistoryMessageList(
            count: count ??
                (widget.selectedConversation.type == ConvType.group.index
                    ? ConversationPerfFlags.groupHistorySdkPageSize
                    : 20),
            direction: direction,
  ...
  Widget build(BuildContext context) {
    ...
    return TIMUIKitChatProviderScope(
      ...
      groupType: widget.selectedConversation.groupType,
